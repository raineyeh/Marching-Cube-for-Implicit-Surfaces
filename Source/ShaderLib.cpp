...
ShaderLib::ShaderLib() : handle(0), linked(false) { }
...
void ShaderLib::compileShader( const char * fileName )
  throw( ShaderLibException ) {
...
string ShaderLib::getExtension( const char * name ) {
  string nameStr(name);
...
throw( ShaderLibException )
{
  if( ! fileExists(fileName) )
  {
    ...
    throw ShaderLibException(message);
  }

  if( handle <= 0 ) {
    handle = glCreateProgram();
    if( handle == 0) {
      throw ShaderLibException("Unable to create shader program.");
    }
  }
...
void ShaderLib::compileShader( const string & source, 
    GLSLShader::GLSLShaderType type,
    const char * fileName )
throw(ShaderLibException)
{
...
void ShaderLib::link() throw(ShaderLibException)
{
  if( linked ) return;
  if( handle <= 0 ) 
    throw ShaderLibException("Program has not been compiled.");
  ...
  if( GL_FALSE == status ) {
    ...
    throw ShaderLibException(string("Program link failed:\n") + logString);
  } else {
...
void ShaderLib::use() throw(ShaderLibException)
{
  if( handle <= 0 || (! linked) ) 
    throw ShaderLibException("Shader has not been linked");
  glUseProgram( handle );
}

int ShaderLib::getHandle()
{
  return handle;
}

bool ShaderLib::isLinked()
{
  return linked;
}

void ShaderLib::bindAttribLocation( GLuint location, const char * name)
{
  glBindAttribLocation(handle, location, name);
}

void ShaderLib::bindFragDataLocation( GLuint location, const char * name )
{
  glBindFragDataLocation(handle, location, name);
}

void ShaderLib::setUniform( const char *name, float x, float y, float z)
{
  GLint loc = getUniformLocation(name);
  glUniform3f(loc,x,y,z);
}

void ShaderLib::setUniform( const char *name, const vec3 & v)
{
  this->setUniform(name,v.x,v.y,v.z);
}

void ShaderLib::setUniform( const char *name, const vec4 & v)
{
  GLint loc = getUniformLocation(name);
  glUniform4f(loc,v.x,v.y,v.z,v.w);
}

void ShaderLib::setUniform( const char *name, const vec2 & v)
{
  GLint loc = getUniformLocation(name);
  glUniform2f(loc,v.x,v.y);
}

void ShaderLib::setUniform( const char *name, const mat4 & m)
{
  GLint loc = getUniformLocation(name);
  glUniformMatrix4fv(loc, 1, GL_FALSE, &m[0][0]);
}

void ShaderLib::setUniform( const char *name, const mat3 & m)
{
  GLint loc = getUniformLocation(name);
  glUniformMatrix3fv(loc, 1, GL_FALSE, &m[0][0]);
}

void ShaderLib::setUniform( const char *name, float val )
{
  GLint loc = getUniformLocation(name);
  glUniform1f(loc, val);
}

void ShaderLib::setUniform( const char *name, int val )
{
  GLint loc = getUniformLocation(name);
  glUniform1i(loc, val);
}

void ShaderLib::setUniform( const char *name, GLuint val )
{
  GLint loc = getUniformLocation(name);
  glUniform1ui(loc, val);
}

void ShaderLib::setUniform( const char *name, bool val )
{
  int loc = getUniformLocation(name);
  glUniform1i(loc, val);
}

void ShaderLib::printActiveUniforms() {
  GLint numUniforms = 0;
...
void ShaderLib::printActiveUniformBlocks() {
  GLint numBlocks = 0;
...
void ShaderLib::printActiveAttribs() {
  GLint numAttribs;
...
const char * ShaderLib::getTypeString( GLenum type ) {
...
void ShaderLib::validate() throw(ShaderLibException)
{
  if( ! isLinked() ) 
    throw ShaderLibException("Program is not linked");
  ...
  if( GL_FALSE == status ) {
    ...
    throw ShaderLibException(string("Program failed to validate\n") + logString);

  }
}

int ShaderLib::getUniformLocation(const char * name )
{
...
bool ShaderLib::fileExists( const string & fileName )
{
  struct stat info;
  int ret = -1;
